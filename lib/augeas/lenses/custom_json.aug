...
(* json lens adapated from generic lens shipped with augeas *)
(*    tailored for redbox system-config.json to handle commas appropriately on output*)
...
let fix_value (value:lens) =
  let array = [ label "array" . lbrack . (value)* . rbrack ] in
  let pair = [ label "entry" . str_store . colon . value ] in
  let obj = [ label "dict" . lbrace . (pair)* . rbrace ] in
